...
Das Installationsskript (s. Anhang \ref{ah_skript}: \nameref{ah_skript}) dient dazu, die im Projekt verwendeten Softwarekomponenten möglichst ohne großen Aufwand und mit so wenig Nutzereingaben wie möglich zu installieren und einzurichten. 
...
\noindent So besteht die Möglichkeit, beispielsweise die Installation von Docker oder dem Systemupdate abzulehnen (vgl. \ref{ah_skript}: \nameref{ah_skript} Zeile 34 ff).
Gibt der Nutzer keinen Operanden oder einen ungültigen Operanden an, wird ein Hilfetext mit der Funktion \texttt{show\_help} (vgl. \ref{ah_skript}: \nameref{ah_skript} Zeile 34ff) ausgegeben, der den Nutzer über die Operatoren, sowie eine allgemeine Verwendung des Skripts informiert.\\
...
\noindent Ob die oben genannten Abhängigkeiten vorhanden sind, wird bei der Installation unabhängig von der Nutzereingabe geprüft und gegebenenfalls bei Fehlen dieser nachinstalliert (vgl. \ref{ah_skript}: \nameref{ah_skript} Zeile 85ff \& \ref{ah_skript} Zeile 297ff: \nameref{ah_skript}).\\
...
\noindent Für Docker wird ein Skript von docker.com heruntergeladen und ausgeführt (vgl. \ref{ah_skript}: \nameref{ah_skript} Zeile 108ff).
